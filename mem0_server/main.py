...
def search_memories(search_req: SearchRequest, _api_key: Optional[str] = Depends(verify_api_key)):
    try:
        ...
        if search_req.top_k is not None:
            extra["top_k"] = search_req.top_k
        if search_req.threshold is not None:
            extra["threshold"] = search_req.threshold
        
        logging.info(f"Search query='{search_req.query}' filters={filters} extra={extra}")
        result = MEMORY_INSTANCE.search(query=search_req.query, filters=filters, **extra)
